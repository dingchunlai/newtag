class HejiaIndex < Hejia::Db::Index
  ...
  class << self
    ...
    def zhuanqu_list_memkey(keyword_id, entity_type_id, type = 'records')
      "zhuanqu_list_#{keyword_id}_#{entity_type_id}_#{type}_22"
    end
    ...
    def get_zhuanqu_list(keyword_id, board_id, entity_type_id, curpage, per_page = 6)
      ...
      if curpage.to_i == 1
        ...
        Hejia::Cache.fetch(memkey, 1.day) do
          zhuanqu_list(keyword_id, entity_type_id, curpage, per_page)
        end
      else
        #第2页起不使用缓存
        zhuanqu_list(keyword_id, entity_type_id, curpage, per_page)
    ...
    def zhuanqu_list(keyword_id, entity_type_id, page, per_page = 6)
      conditions = []
      conditions << "i.entity_id = r.entity_id and r.keyword_id = #{keyword_id} and r.relation_type = #{entity_type_id} and i.entity_type_id = #{entity_type_id} and i.entity_id = a.ID and a.STATE=0"
      HejiaIndex.paginate(
        ...
        :joins => "i, relations r, 51hejia.HEJIA_ARTICLE a",
    ...
    def get_zhuanqu_list_counter(keyword_id, entity_type_id)
      ...
      Hejia::Cache.fetch(memkey, 3.days) do
        conditions = []
        conditions << "i.entity_id = r.entity_id and r.keyword_id = #{keyword_id} and r.relation_type = #{entity_type_id} and i.entity_type_id = #{entity_type_id} and i.entity_id = a.ID and a.STATE=0"
        HejiaIndex.count("i.id", :conditions => conditions.join(" and "), :joins => "i, relations r, 51hejia.HEJIA_ARTICLE a")
      end
    end
